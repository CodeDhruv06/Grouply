...
export default function PayPage() {
  ...
  async function fetchReceiver() {
    try {
      const res = await fetch(`http://localhost:5000/api/v1/profile/receiver/${qrCodeId}`);
  ...
  async function handlePayment() {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/v1/profile/pay/${qrCodeId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ senderEmail, amount, note }),
      });
      const data = await res.json();
